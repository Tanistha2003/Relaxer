...
const moment = require('moment') //for dates and time manipulation
...
class userController {
    ...
    static async scheduler() {
        ...
        cron.schedule('0 0 * * *', async () => {// min,hour,date,month,day of week (midnight everyday)
            //sets up cron job
               
                let d = new Date()
                let day = d.getDay()
                //curr date, time and day of week when job runs
                ...
                const users = await userSchema.find() //find users

                if(users){
                    const currdate = moment().format('YYYY-MM-DD')
                    const currentDate = new Date(currdate)

                for (let i = 0; i < users.length; i++) { //for each user

                    const useremail = users[i].email
                    console.log(useremail)
                    const addactivity = await new activitySchema() //activity for each user
    ...
    async saveUser(req) {
        ...
        try {
            ...
            if (req.body.email) {
                const userExist = await userSchema.findOne({ email: req.body.email }) //find the user by email
                let obj = {
                    0: "Sunday",
                    1: "Monday",
                    2: "Tuesday",
                ...
                if (userExist == null) { //no such user

                    const newuser = await new userSchema() //create user
    ...
    async weeklyactivity(req) {
        try {
            ...
            const user = await userSchema.findOne({ _id: req.userid }) //as id is unique for all users
            if (user) {
                let emailId = user.email
                let date = moment()
                let start = moment().startOf('isoweek') // Monday 00:00:00
                let end = moment().endOf('isoweek')//Sunday 23:59:59
                ...
                })//weekly activity finding, where only the email id of the user is displayed
    ...
    async getbadges(req) {
        ...
        try {
            ...
            if (user) {
                ...
                const activity = await activitySchema.find({
                    date: {
                        $gte: start, //start day of the month at midnight
                        $lte: end
                    }, email: emailId
                })
               ...
               // console.log(activity[0].activity.water)
                ...
                for (let i = 0; i < activity.length; i++) { //all activity documents within the month

                    watercount = watercount + activity[i].activity.water //nested bson
                ...
                console.log(req.query.stepsCount, req.query.heartPoints, req.query.caloriesBurned) //GET requests

                const finduser = await badgeSchema.findOne({ email: emailId, month: month }) 
                //find badge details of user by their email id and for the curr month
                ...
                if (finduser == null) {
                    ...
                    if (watercount >= 240) { //8 glasses per day of 30 days

                        adduser.waterBadge = true
                    }
                    if (meditationcount >= 27000) { // 15 mins for 30 days


                        adduser.meditationBadge = true
                    }
                    if (exercisecount >= 108000) { // 1hr for 30 days
                ...
                else { //if user already has badges for the month
    ...
    async leaderboard(req) { //find user posn in leaderboard based on no. of badges for curr month

        const currentMonth = moment().format('MMMM'); //Full name of month

        const data = await badgeSchema.find({
            month: currentMonth
        }) //data of all users for the current month
        ...
        let arr = [] // array of objects to store users with the no. of badges they have
...
userController.scheduler() //call scheduler function to start the cron job
